...
directory. It holds a single package, so it is refused wherever installing
means installing a whole dependency graph: always for `add`, `sync` and
`remove`, and for `install --remote` — which performs a Fetch — the moment the
fetched repository turns out to have dependencies of its own. `install`'s
working-tree form (`install <path>`) and `uninstall` always accept one; they
never touch more than one package to begin with.
