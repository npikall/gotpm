...
Delete a package from the store, whoever put it there. This is the counterpart
of `install`; to drop a dependency of the current project, use
[`remove`](#remove) instead, which updates `typst.toml` and `gotpm.lock` as well.
