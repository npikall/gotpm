...
func TestWalk_DependencyWithoutALock(t *testing.T) { //nolint: paralleltest
	...
	assert.Contains(t, err.Error(), leaf.Import(), "the error names what could not be resolved")
	assert.Contains(t, err.Error(), middle.Import(), "and the package that must fix it")
...
func TestWalk_DependencyLockMissingAnEntry(t *testing.T) { //nolint: paralleltest
	...
	require.ErrorIs(t, err, depgraph.ErrUnresolvable)
	assert.Contains(t, err.Error(), forgotten.Import())
	assert.NotContains(t, err.Error(), known.Import())
}
