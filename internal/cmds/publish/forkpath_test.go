...
// TestDefaultForkPathIgnoresCase covers a fork url the user typed with
// different capitalisation than the clone's origin has. The hosts these forks
// live on treat that as one repository, so deriving two locations from it would
// clone the same fork twice.
func TestDefaultForkPathIgnoresCase(t *testing.T) { //nolint: paralleltest // Isolate uses t.Setenv
	testrepo.Isolate(t)

	lower, err := publish.DefaultForkPath("https://github.com/me/packages")
	require.NoError(t, err)
	upper, err := publish.DefaultForkPath("https://GitHub.com/Me/Packages")
	require.NoError(t, err)

	assert.Equal(t, lower, upper)
}

// TestDefaultForkPathKeepsLocalCase is the other half of the case rule: a fork
// on this machine is named by a path, and the filesystems gotpm runs on tell
// two paths of differing case apart, so folding them would make one clone serve
// two repositories.
func TestDefaultForkPathKeepsLocalCase(t *testing.T) { //nolint: paralleltest // Isolate uses t.Setenv
	testrepo.Isolate(t)

	lower, err := publish.DefaultForkPath("file:///srv/packages")
	require.NoError(t, err)
	upper, err := publish.DefaultForkPath("file:///srv/Packages")
	require.NoError(t, err)

	assert.NotEqual(t, lower, upper)
}

// TestResolveForkPathKeepsLegacyCloneWhenDerivedExists covers an interrupted
// upgrade: a clone already sits at the derived location, so the legacy one is
// not moved over it.
func TestResolveForkPathKeepsLegacyCloneWhenDerivedExists(t *testing.T) { //nolint: paralleltest // Isolate uses t.Setenv
	testrepo.Isolate(t)
	origin := setupOriginRepo(t, []string{pkgDir})
	legacy := legacyClone(t, origin)
	derived, err := publish.DefaultForkPath(cloneURL(origin))
	require.NoError(t, err)
	require.NoError(t, paths.EnsureDir(derived))

	forkPath, err := publish.ResolveForkPath(testLogger(), &config.Config{}, cloneURL(origin))
	require.NoError(t, err)

	assert.Equal(t, derived, forkPath)
	assert.DirExists(t, filepath.Join(legacy, ".git"), "the legacy clone is left where it is")
}
